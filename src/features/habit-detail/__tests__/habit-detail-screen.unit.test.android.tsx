...
describe("going somewhere else", () => {
  it("should open the habit form from the header action", async () => {
    const habit = seedHabit();
    ...
    await fireEvent.press(getByLabelText(habitDetail.editHabit));
  ...
  it("should give the header a surface of its own, so nothing scrolls under it", async () => {
    const habit = seedHabit();

    await renderDetail(habit.id);

    expect(headerOptions().headerTransparent).toBe(false);
  });
