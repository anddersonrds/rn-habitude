...
/** The inset behaviour the tree asked for, which on Android is none at all. */
function insetsAsked(container: TestInstance): string[] {
  return container
    .queryAll((node) => node.props.contentInsetAdjustmentBehavior != null)
    .map((node) => node.props.contentInsetAdjustmentBehavior as string);
}
...
describe("the habit being shown", () => {
  ...
  it("should leave the top of the content to the header rather than an inset", async () => {
    const habit = seedHabit();

    const { container } = await renderHistory(habit.id);

    expect(insetsAsked(container)).toEqual([]);
  });
